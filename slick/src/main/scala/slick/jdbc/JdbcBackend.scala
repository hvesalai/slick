...
trait JdbcBackend extends RelationalBackend {
  ...
  abstract class JdbcDatabaseDef[F[_]](val source: JdbcDataSource) extends BasicDatabaseDef[F] {
    ...
    override protected def sessionAsContext(session: Session, state: ExecState): Context = {
      ...
      new JdbcActionContext {
        ...
        override def statePinned: Boolean = pinned
      }
    }
    ...
    override protected def streamFromSDA[T](
      ...
      //   Some(null) = not yet started (initial state)
      //   Some(iter) = in-progress iterator
      //   None       = exhausted (stop unfolding)
      Stream.unfoldEval[F, Option[sda.StreamState], T](Some(null.asInstanceOf[sda.StreamState])) {
        case None =>
        ...
        case Some(state) =>
          F.blocking {
            ...
            val ctx2 = new JdbcStreamingActionContext {
              override def rawSession: JdbcSessionDef = outerSession
              override def transactionDepth: Int = outerTransactionDepth
              override def statePinned: Boolean = outerPinned
              override def emit(v: Any): Unit = {
                // For ResultSetMutator elements we must capture the value of `end` at emit
                // time, before emitStream() sets state=2 in the same F.blocking call.
                // Without this, the consumer sees state=2 and `end` throws.
                val captured: T = v match {
                  case m: ResultSetMutator[_] =>
                    // Capture `end` now (state == 1 for end-marker, 0 for normal row)
                    val frozenEnd: Boolean = try m.end catch { case _: SlickException => true }
                    val delegate = m.asInstanceOf[ResultSetMutator[Any]]
                    (new ResultSetMutator[Any] {
                      def row: Any               = delegate.row
                      def row_=(value: Any): Unit = delegate.row = value
                      def += (value: Any): Unit   = delegate += value
                      def delete: Unit            = delegate.delete
                      def end: Boolean            = frozenEnd
                    }).asInstanceOf[T]
                  case _ =>
                    v.asInstanceOf[T]
                }
                emitted = Some(captured)
              }
            }
            val nextRaw = sda.emitStream(ctx2, 1L, state)
            emitted match {
              case Some(v) =>
                // nextRaw == null means the iterator is exhausted after this element;
                // wrap in None so the next unfoldEval step terminates.
                val nextState: Option[sda.StreamState] =
                  if (nextRaw eq null) None else Some(nextRaw)
                Some((v, nextState))
              case None =>
                // emitStream was called but emitted nothing → stream is done
                None
            }
          }
      }
  ...
  trait DatabaseFactoryDef {
    ...
    def forSource[G[_]](source: JdbcDataSource, maxConnections: Option[Int] = None)(implicit AG: Async[G]): Resource[G, Database[G]] = {
      ...
      )(db => AG.delay(db.close())).evalTap { _ =>
        // For keepAliveConnection sources, open the keepalive connection eagerly during
        // Resource acquire so that named in-memory databases (e.g. H2) are not destroyed
        // before the first DBIO action runs.
        source match {
    ...
    /** Create a Database from a `javax.sql.DataSource`.
      ...
      * @param maxConnections  The maximum number of concurrent connections (used for back-pressure).
      * @param keepAliveConnection Keep an extra connection open for named in-memory databases (e.g. H2 tests).
      */
    ...
    /** Load a database configuration through [[https://github.com/typesafehub/config Typesafe Config]].
      ...
      *   <li>`numThreads` (Int, default 20) — used as default `maxConnections`</li>
